...
class classA_U1FGTN:
    ...
    def markov_meas_feedback(self, G, Rx, Ry, n_a=0.5, local_mode=False, p_meas=1.0, forced_meas=False):
        ...
        If forced_meas is True, measurement outcomes use uniform coin flips (1/2, 1/2),
        bypassing Born-rule probabilities.
        '''
        G = np.asarray(G, dtype=np.complex128)
    ...
    def post_selection_markov_top_layer(self, G, Rx, Ry, p_meas=1.0, local_mode=False):
        """
        ...
        if local_mode:
            idx_mu1 = 0 + 2 * Rx + 2 * self.Nx * Ry  # expected unoccupied
            idx_mu2 = 1 + 2 * Rx + 2 * self.Nx * Ry  # expected occupied
            e_mu1 = Il[idx_mu1]
            e_mu2 = Il[idx_mu2]
            P_mu1 = np.outer(e_mu1, e_mu1.conj())
            P_mu2 = np.outer(e_mu2, e_mu2.conj())
            ...
            if p_meas >= 1.0 or np.random.rand() < p_meas:
                G = self.measure_only_top_layer(G, P_mu1, particle=False)
            if p_meas >= 1.0 or np.random.rand() < p_meas:
                G = self.measure_only_top_layer(G, P_mu2, particle=True)
            return G
        ...
        if p_meas >= 1.0 or np.random.rand() < p_meas:
            G = self.measure_only_top_layer(G, P_Ap, particle=False)  # Ap unocc
        if p_meas >= 1.0 or np.random.rand() < p_meas:
            G = self.measure_only_top_layer(G, P_Bp, particle=False)  # Bp unocc
        if p_meas >= 1.0 or np.random.rand() < p_meas:
            G = self.measure_only_top_layer(G, P_Am, particle=True)   # Am occ
        if p_meas >= 1.0 or np.random.rand() < p_meas:
            G = self.measure_only_top_layer(G, P_Bm, particle=True)   # Bm occ
        return G
    ...
    def _sequence_helper(self, sequence, Nx=None, Ny=None, rng=None, dw_exclude=None, top_region_last=False, skip_trivial=False):
        ...
        sequence : str
            ...
            - "dw_symmetric_y": Center Y-scan, then Right-then-Left X-scan. For every x-column,
              scans from the lattice center in y (upward then downward). X starts at center,
              sweeps to the right edge, then restarts at center and sweeps to the left edge.
        ...
        allowed = (
            ...
            "random", "dw_symmetric", "dw_symmetric_2", "dw_symmetric_y", "spiral",
            "random_dw_symmetric", "dw_symmetric_random",
        )
        ...
        if mode == "snake_y":
            ...
            for Rx in x_order:
                for Ry in range(Ny):
                    coords.append((Rx, Ry))
        elif mode in ("dw_symmetric", "dw_symmetric_2", "dw_symmetric_y", "spiral"):
            ...
            def _center_right_then_left_order(xs):
                xs_sorted = sorted(xs)
                if not xs_sorted:
                    return []
                mid_idx = (len(xs_sorted) - 1) // 2
                right = xs_sorted[mid_idx:]
                left = list(reversed(xs_sorted[:mid_idx]))
                return right + left

            def _center_up_then_down_order(n):
                mid = n // 2
                return list(range(mid, n)) + list(range(mid - 1, -1, -1))
            ...
            if mode == "dw_symmetric" and (getattr(self, "DW", False) and hasattr(self, "DW_loc") and len(self.DW_loc) >= 2):
                ...
                y_order = range(Ny)
            elif mode == "dw_symmetric_y" and (getattr(self, "DW", False) and hasattr(self, "DW_loc") and len(self.DW_loc) >= 2):
                dw_sorted = sorted(list(set(self.DW_loc)))
                x_min, x_max = int(dw_sorted[0]), int(dw_sorted[-1])
                slab_xs = range(x_min, x_max + 1)
                left_xs = range(0, x_min)
                right_xs = range(x_max + 1, Nx)
                x_order = (
                    _center_right_then_left_order(slab_xs)
                    + _center_right_then_left_order(left_xs)
                    + _center_right_then_left_order(right_xs)
                )
                y_order = _center_up_then_down_order(Ny)
            else:
                mid_x = Nx // 2
                if mode == "dw_symmetric_2": # Edge-In
                    x_order = list(range(0, mid_x + 1)) + list(range(Nx - 1, mid_x, -1))
                elif mode == "dw_symmetric_y":
    ...
    def run_markov_circuit(
        ...
        at each visited site (no ancilla-swap feedback).
        forced_meas: If True, use uniform (1/2, 1/2) outcome sampling in markov_meas_feedback
        instead of Born-rule probabilities. Ignored when postselect=True.
        top_triv_back_forth: If True (DW must be True), override the x-column ordering
        to start at the topological boundary and scan "downwards" in x, while preserving
        the per-column order defined by `sequence`.
        top_triv_back_forth_local_mode: If True (DW must be True), use local_mode=True
        in the trivial region during the same sweep.
        track_dw_ch: If True, track DW-region Chern marker sum vs cycle.
        coherent_term: Optional Hamiltonian (Nlayer x Nlayer or Ntot x Ntot) applied
        as U = exp(-iH) after each full cycle on the top layer.
        ...
        is_dw_active = getattr(self, "DW", False)
        if track_dw_ch and not (is_dw_active and hasattr(self, "DW_loc") and len(self.DW_loc) >= 2):
            raise ValueError("track_dw_ch requires DW=True with valid DW_loc.")
        ...
        def _cache_key(*, Nx, Ny, cycles, samples, nshell, DW, init_mode, n_a, seq, store_mode, exclude, pm, tbtf, tbtf_lm, ps, fm):
            nsh = "None" if nshell is None else str(nshell)
            ex_str = str(exclude) if exclude is not None else "None"
            key = (
                f"N{int(Nx)}x{int(Ny)}"
        ...
        def _plot_dw_ch_history(dw_ch_hist, samples_count, store_mode):
            if dw_ch_hist is None:
                return None
            outdir = self._ensure_outdir("figs/dw_chern_history")
            key = _cache_key(
                Nx=self.Nx, Ny=self.Ny, cycles=cycles, samples=samples_count,
                nshell=self.nshell, DW=self.DW, init_mode=init_mode,
                n_a=n_a, seq=sequence_mode, store_mode=store_mode,
                exclude=exclude_arg,
                pm=p_meas,
                tbtf=top_triv_back_forth,
                tbtf_lm=top_triv_back_forth_local_mode,
            ...
            x = np.arange(dw_ch_hist.shape[1], dtype=int)
            if samples_count > 1:
                for s in range(samples_count):
                    ax.plot(x, dw_ch_hist[s], color="0.7", lw=1.0, alpha=0.5)
                ax.plot(x, np.mean(dw_ch_hist, axis=0), color="k", lw=2.0, label="avg")
                ax.legend()
            else:
                ax.plot(x, dw_ch_hist[0], color="k", lw=2.0)
            ax.set_xlabel("cycle")
            ax.set_ylabel("DW region Chern sum")
            ax.set_title("DW-region Chern marker sum vs cycle")
        ...
        def _prepare_coherent_unitary(instance):
            if coherent_term is None:
                return None
            H = np.asarray(coherent_term, dtype=np.complex128)
            if H.shape == (instance.Ntot, instance.Ntot):
                H = H[:Nlayer, :Nlayer]
            elif H.shape != (Nlayer, Nlayer):
                raise ValueError(f"coherent_term shape error: {H.shape}")
            if not np.allclose(H, H.conj().T):
                raise ValueError("coherent_term must be Hermitian.")
            evals, vecs = np.linalg.eigh(H)
            U = vecs @ np.diag(np.exp(-1j * evals)) @ vecs.conj().T
            return U

        store_full_history = G_history or track_dw_ch
        ...
        def _run_single(instance, enable_progress, return_eta=False, sample_idx=None, total_samples=None):
            ...
            U_coh = _prepare_coherent_unitary(instance)
            U_dag = None if U_coh is None else U_coh.conj().T
            ...
            def _override_x_order(coords_list):
                if top_triv_x0 is None or top_triv_x1 is None:
                    return coords_list
                x_start = int(top_triv_x1 - 1)
                x_order = list(range(x_start, -1, -1)) + list(range(instance.Nx - 1, x_start, -1))
                by_x = {}
                for Rx, Ry in coords_list:
                    by_x.setdefault(Rx, []).append((Rx, Ry))
                out = []
                for Rx in x_order:
                    if Rx in by_x:
                        out.extend(by_x[Rx])
            ...
            if use_top_triv:
                ...
                total_sites = (cycles_eff * coords_len)
                desc = "Markov RAC (DW boundary sweep)"
            else:
                coords_len = len(coords_for_len)
                total_sites = (cycles_eff * coords_len)
            ...
            if use_top_triv:
                ...
                for _c in range(cycles_eff):
                    global_cycle += 1
                    iter_coords = _override_x_order(iter_fn())
                    for Rx, Ry in iter_coords:
                        local_mode = False
                        if triv_local_mode and top_triv_x0 is not None and top_triv_x1 is not None:
                            local_mode = not (top_triv_x0 <= Rx < top_triv_x1)
                        if postselect:
                            G_top = instance.post_selection_markov_top_layer(
                                G_top, Rx, Ry, p_meas=p_meas, local_mode=local_mode
                            )
                        else:
                            G_top = instance.markov_meas_feedback(
                                G_top, Rx, Ry, n_a=n_a, local_mode=local_mode, p_meas=p_meas, forced_meas=forced_meas
                            )
                        if pbar is not None:
                            pbar.update(1)
                            pbar.set_postfix_str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), refresh=False)
                            _capture_eta()
                    if U_coh is not None:
                        G_top = U_coh @ G_top @ U_dag
                    if store_full_history:
                        if history_stride is None or (global_cycle % history_stride) == 0:
                            history.append(G_top.copy())
                            last_saved_cycle = global_cycle
            else:
                for _c in range(cycles_eff):
                    iter_coords = iter_fn()
                    for Rx, Ry in iter_coords:
                        if postselect:
                            G_top = instance.post_selection_markov_top_layer(
                                G_top, Rx, Ry, p_meas=p_meas
                            )
                        else:
                            G_top = instance.markov_meas_feedback(
                                G_top, Rx, Ry, n_a=n_a, p_meas=p_meas, forced_meas=forced_meas
                            )
            ...
            if store_full_history and history_stride is not None and use_top_triv:
                if last_saved_cycle != cycles_eff:
                    history.append(G_top.copy())
            ...
            if track_dw_ch:
                if history:
                    dw_ch_history = np.array(
                        [instance.sum_chern_marker_dw_region(state) for state in history],
                        dtype=float
                    )
                else:
                    dw_ch_history = np.empty((0,), dtype=float)

            if return_eta:
                if track_dw_ch:
                    return result, (last_eta if last_eta is not None else None), dw_ch_history
                return result, (last_eta if last_eta is not None else None)
            if track_dw_ch:
                return result, dw_ch_history
            return result
        ...
        if not parallelize_samples:
            ...
            dw_ch_histories = [] if track_dw_ch else None
            total_samples = samples
            _emit_save_notice(total_samples)
            for idx in range(total_samples):
                if progress:
                    if track_dw_ch:
                        single_result, eta, dw_ch_single = _run_single(
                    ...
                    if track_dw_ch:
                        single_result, dw_ch_single = _run_single(
                ...
                if dw_ch_histories is not None: dw_ch_histories.append(dw_ch_single)
            ...
            if G_history:
                ...
                if track_dw_ch:
                    dw_ch_hist = np.stack(dw_ch_histories, axis=0)
                    result["dw_ch_hist"] = dw_ch_hist
                    result["dw_ch_hist_avg"] = np.mean(dw_ch_hist, axis=0)
                    result["dw_ch_plot_path"] = _plot_dw_ch_history(
                        dw_ch_hist, samples_count=dw_ch_hist.shape[0], store_mode="history"
                    )
                return result
            ...
            if track_dw_ch:
                ...
                result["dw_ch_hist_avg"] = np.mean(dw_ch_hist, axis=0)
                result["dw_ch_plot_path"] = _plot_dw_ch_history(
                    dw_ch_hist, samples_count=dw_ch_hist.shape[0], store_mode="final"
                )
            return result
        ...
        def _worker(seed_u32):
            with threadpool_limits(limits=1):
                ...
                result = child.run_markov_circuit(
                    ...
                    cycles=cycles, 
                    postselect=postselect,
                    forced_meas=forced_meas,
                    samples=1,
                    parallelize_samples=False, init_mode=init_mode,
                    G_init=child.G0,
                    save_final_G_only=False, save=False, n_a=n_a,
                    save_init=save_init,
                    save_history_stride=history_stride_outer,
                    sequence=sequence_mode,
                    dw_exclude=exclude_arg,
                    p_meas=p_meas,
                    top_triv_back_forth=top_triv_back_forth,
                    top_triv_back_forth_local_mode=top_triv_back_forth_local_mode,
                    track_dw_ch=track_dw_ch,
                    coherent_term=coherent_term,
                )
                if G_history:
                    G_out = np.asarray(result["G_hist"])[0]
                else:
                    G_out = np.asarray(result["G_final"])[0]
                if track_dw_ch:
                    dw_out = np.asarray(result["dw_ch_hist"])[0]
                    return G_out, dw_out
                return G_out
                ...
                with parallel_backend("loky", n_jobs=workers, inner_max_num_threads=1):
                    with threadpool_limits(limits=1):
                        return Parallel(n_jobs=workers)(
                            delayed(_worker)(seeds[i]) for i in range(start_idx, start_idx + count)
                        )
            os.environ.setdefault("OMP_NUM_THREADS", "1")
            with threadpool_limits(limits=1):
        ...
        dw_ch_list = None
        if track_dw_ch:
            G_hist_list, dw_ch_list = zip(*G_hist_list)
            G_hist_list = list(G_hist_list)
            dw_ch_list = list(dw_ch_list)
        ...
        if G_history:
            ...
            if track_dw_ch:
                dw_ch_hist = np.stack(dw_ch_list, axis=0)
                result["dw_ch_hist"] = dw_ch_hist
                result["dw_ch_hist_avg"] = np.mean(dw_ch_hist, axis=0)
                result["dw_ch_plot_path"] = _plot_dw_ch_history(
                    dw_ch_hist, samples_count=dw_ch_hist.shape[0], store_mode="history"
    ...
    def run_markov_channel(
        ...
        def _cache_key(*, Nx, Ny, sweeps, nshell, DW, init_mode, n_a, p, exclude, tbtf_mode):
            nsh = "None" if nshell is None else str(nshell)
            ex_str = str(exclude) if exclude is not None else "None"
            return (
                f"N{int(Nx)}x{int(Ny)}"
        ...
        if trivial_product_state_DW:
            ...
            for _ in range(sweeps):
                _execute_sweep(_override_x_order(iter_fn()))
                if G_history: history.append(G_top.copy())
        else:
            for _ in range(sweeps):
                _execute_sweep(iter_fn())
